'use client';
// components/maintenance/MaintenanceReportForm.tsx
// The maintenance report FORM, extracted from app/maintenance/report so it can
// be reused both on the public page and inside the landlord portal (chrome is
// supplied by each page). Handles its own state, Cloudinary uploads, PDF and
// submit to /api/maintenance. `prefill` seeds the fields; `onSubmittedChange`
// lets the host page react (e.g. hide its hero on success).
...
async function uploadToCloudinary(file: File): Promise<string> {
  const sigRes = await fetch('/api/cloudinary-sign', {
    ...
    body: JSON.stringify({ folder: CLOUDINARY_FOLDERS.maintenance }),
  });
...
export type MaintenancePrefill = Partial<{ fullName: string; email: string; contactNumber: string; postcode: string; addressLine1: string }>;
...
export default function MaintenanceReportForm({
  ...
  useEffect(() => { onSubmittedChange?.(submitted); }, [submitted, onSubmittedChange]);
    ...
    <div style={{ maxWidth: 720, margin: '0 auto' }}>
      ...
      <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 2px 16px rgba(0,0,0,0.07)', padding: isMobile ? '24px 16px' : '40px 48px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div>
          ...
          <p style={sectionSubStyle}>At least 3 photos are required. You can add photos one at a time, and more angles help us diagnose faster.</p>
        </div>
        <FileUpload
